            ...
            <form method="POST" action="{{ route('product.update', $product->product_id) }}" enctype="multipart/form-data">
                ...
                <div class="row mb-4">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="barcode" class="form-control bg-white" value="{{ $product->barcode }}"
                               required />
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Product Name</label>
                        <input type="text" name="product_name" class="form-control bg-white"
                               value="{{ $product->product_name }}" required />
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Price</label>
                        <input type="number" step="0.01" name="price" class="form-control bg-white"
                               value="{{ $product->price }}" required />
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Minimum Stock Level</label>
                        <input type="number" name="min_stock" class="form-control bg-white"
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="country_id" class="form-control bg-white" required>
                            <option value="">Select Country</option>
                            @foreach ($countries as $country)
                                <option value="{{ $country->country_id }}"
                                    {{ $product->country_id == $country->country_id ? 'selected' : '' }}>
                    ...
                    <div class="col-12 mb-3">
                        ...
                        <textarea name="description" class="form-control bg-white" style="width: 100%;">{{ $product->description }}</textarea>
                    </div>
                    <div class="col-md-6 mb-3 d-flex align-items-center">
                        <div>
                            <label class="form-label fw-bold">Image</label>
                            <div class="d-flex align-items-center mt-2">
                                <img src="{{ $product->image ? asset('assets/image/' . $product->image) : asset('assets/image/default.png') }}"
