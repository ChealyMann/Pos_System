            ...
            <form action="{{ route('user.update', $user->user_id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="usercode" value="{{ old('usercode', $user->usercode) }}" class="form-control bg-white" required>
                    </div>

                    <!-- Name -->
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Name</label>
                        <input type="text" name="user_name" value="{{ old('user_name', $user->user_name) }}" class="form-control bg-white" required>
                    </div>

                    <!-- Email -->
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Email</label>
                        <input type="email" name="email" value="{{ old('email', $user->email) }}" class="form-control bg-white" required>
                    </div>

                    <!-- Phone -->
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Phone</label>
                        <input type="text" name="phone_number" value="{{ old('phone_number', $user->phone_number) }}" class="form-control bg-white">
                    </div>

                    <!-- Password -->
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-bold">Password</label>
                        <input type="password" name="password" class="form-control bg-white" placeholder="Leave blank to keep old password">
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="d-flex align-items-center mt-2">
                            <img id="imagePreview"
                                 src="{{ $user->image ? asset('assets/image/'.$user->image) : asset('assets/image/noprofile.png') }}"
                                 alt="User" class="rounded-circle"
                                 style="width:70px; height:70px; object-fit:cover; border:2px solid #fff; box-shadow:0 2px 8px rgba(0,0,0,0.08);">
                            <input type="file" name="image" class="form-control ms-3" style="max-width:220px;" onchange="previewImage(event)">
                        </div>
                    </div>
