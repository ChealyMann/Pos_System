...
<div class="col-lg-10 col-md-10 px-4 py-3" style="background:#f5f5f5; min-height:100vh;">
            ...
            <div class="d-flex align-items-center" style="margin-left: auto; gap: 16px;">
                <form method="GET" class="d-flex align-items-center">
                    <select name="filter" class="form-select me-3" style="max-width:180px;">
                        ...
                        <option value="in_stock" {{ request('filter') == 'in_stock' ? 'selected' : '' }}>In Stock
                        </option>
                        <option value="out_stock" {{ request('filter') == 'out_stock' ? 'selected' : '' }}>Out Stock
                        </option>
                    </select>
    ...
    <div class="modal fade" id="addStockModal" tabindex="-1" aria-labelledby="addStockModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="background:#e4e4e4; border-radius:32px; min-width:900px; padding:20px;">
                ...
                <form action="{{ route('stock_in.store') }}" method="POST" id="stockForm">
                    @csrf
                    <div class="row g-4">
                        <div class="col-md-6">
                            <label for="purchase_id" class="form-label">Purchase <span
                                    class="text-danger">*</span></label>
                            <select name="purchase_id" id="purchase_id"
                                class="form-control bg-white @error('purchase_id') is-invalid @enderror">
                                <option value="">-- Select Purchase --</option>
                                @foreach($purchases as $purchase)
                                <option value="{{ $purchase->purchase_id }}"
                                    {{ old('purchase_id') == $purchase->purchase_id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="product_id" id="product_id"
                                class="form-control bg-white @error('product_id') is-invalid @enderror">
                        ...
                        <div class="col-md-6">
                            <label for="cost_per_item" class="form-label">Cost <span class="text-danger">*</span></label>
                            <input type="number" step="0.01"
                                class="form-control @error('cost_per_item') is-invalid @enderror" id="cost_per_item"
                                name="cost_per_item" value="{{ old('cost_per_item') }}">
                            @error('cost_per_item')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <table class="table align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ url('stock_in/' . $stock->product_id) }}"
                    ...
                    @foreach($stocks as $stock)
                    @if ($stock->total_qty_in_stock <= 0) <tr class="table-danger">
                        <td>{{ $stock->barcode }}</td>
                        <td>{{ $stock->product_name }}</td>
                        <td>{{ number_format($stock->avg_cost, 2) }}</td>
                        <td>{{ $stock->total_qty_in_stock }}</td>
    ...
    <script>
        ...
        $('#purchase_id').on('change', function() {
            ...
            $('#cost_per_item, #qty').val('');
            if (purchase_id) {
                $.ajax({
                    url: "{{ route('get_product.by_purchase') }}",
                    type: "POST",
                    data: {
                    ...
                    success: function(response) {
                        console.log(response); // Debug JSON
        ...
        $('#product_id').on('change', function() {
            ...
            let selectedProduct = purchaseProducts.find(p => p.product_id == selectedId);
            if (selectedProduct) {
                $('#cost_per_item').val(selectedProduct.unit_cost);
                $('#qty').val(selectedProduct.qty);
            } else {
                $('#cost_per_item, #qty').val('');
            }
        });
